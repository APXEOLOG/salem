...
public class AutoReconnect extends GenericAutoService
{
	...
	{
		ALS.alDebugPrint("AutoReconnect: Update with state " + state);
		...
		//ALS.alDebugPrint("AutoReconnect: Returned from " + state);
	}
	...
	{
		SChatWrapper.ircLogToChat("Failed to recoonect.");
		this.attempt = 0;
	}
	...
	{
		//ALS.alDebugPrint("AutoReconnect: Error connecting: " + error);
		...
		{
			SChatWrapper.ircLogToChat("Tried reconnect " + attempt + " times, giving up.");
			finalFailure();
			return false;
		}
	...
	protected void updateCommand( InCommand command )
	{
		...
		{
			//ALS.alDebugPrint("AutoReconnect: Disabling due to receiving own QUIT.");
			disable();
		}
	}
