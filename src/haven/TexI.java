...
public class TexI extends TexGL {
	public static ComponentColorModel glcm = new ComponentColorModel(
	...
	private int detectfmt(BufferedImage img) {
		...
		if (!(img.getSampleModel() instanceof PixelInterleavedSampleModel))
			return (-1);
		PixelInterleavedSampleModel sm = (PixelInterleavedSampleModel) img
				.getSampleModel();
	...
	protected void fill(GOut g) {
		...
		if ((ifmt == GL.GL_RGBA) || (ifmt == GL.GL_BGRA)) {
			byte[] pixels = ((DataBufferByte) back.getRaster().getDataBuffer())
					.getData();
			if (sz.equals(tdim)) {
				gl.glTexImage2D(GL.GL_TEXTURE_2D, 0, fmt, tdim.x, tdim.y, 0,
						ifmt, GL.GL_UNSIGNED_BYTE, ByteBuffer.wrap(pixels));
				if (mipmap)
					genmipmap(gl, 1, tdim, pixels, ifmt);
			} else {
				gl.glTexImage2D(GL.GL_TEXTURE_2D, 0, fmt, tdim.x, tdim.y, 0,
						ifmt, GL.GL_UNSIGNED_BYTE, null);
				gl.glTexSubImage2D(GL.GL_TEXTURE_2D, 0, 0, 0, sz.x, sz.y, ifmt,
			...
			gl.glTexImage2D(GL.GL_TEXTURE_2D, 0, fmt, tdim.x, tdim.y, 0,
					GL.GL_RGBA, GL.GL_UNSIGNED_BYTE, ByteBuffer.wrap(pixels));
			if (mipmap)
	...
	private void genmipmap(GL gl, int lev, Coord dim, byte[] data, int ifmt) {
		...
		gl.glTexImage2D(GL.GL_TEXTURE_2D, lev, fmt, dim.x, dim.y, 0, ifmt,
				GL.GL_UNSIGNED_BYTE, ByteBuffer.wrap(ndata));
		if ((dim.x > 1) || (dim.y > 1))
	...
	public static BufferedImage mkbuf(Coord sz) {
		WritableRaster buf = Raster.createInterleavedRaster(
				DataBuffer.TYPE_BYTE, sz.x, sz.y, 4, null);
		BufferedImage tgt = new BufferedImage(glcm, buf, false, null);
		return (tgt);
	}

	public static byte[] convert(BufferedImage img, Coord tsz, Coord ul,
			Coord sz) {
		WritableRaster buf = Raster.createInterleavedRaster(
				DataBuffer.TYPE_BYTE, tsz.x, tsz.y, 4, null);
		BufferedImage tgt = new BufferedImage(glcm, buf, false, null);
		Graphics g = tgt.createGraphics();
		g.drawImage(img, 0, 0, sz.x, sz.y, ul.x, ul.y, ul.x + sz.x,
				ul.y + sz.y, null);
		g.dispose();
