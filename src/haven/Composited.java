...
public class Composited implements Rendered {
	...
	private static final Rendered.Order modorder = new Rendered.Order<Model.Layer>() {
		...
		private final Rendered.RComparator<Model.Layer> cmp = new Rendered.RComparator<Model.Layer>() {
			public int compare(Model.Layer a, Model.Layer b, GLState.Buffer sa,
					GLState.Buffer sb) {
				if (a.z1 != b.z1)
					return (a.z1 - b.z1);
				return (a.z2 - b.z2);
	...
	private abstract class Equ implements Rendered {
		...
		private Equ(ED ed) {
			Skeleton.BoneOffset bo = ed.res.get().layer(
					Skeleton.BoneOffset.class, ed.at);
			GLState bt;
			if (bo != null) {
				bt = bo.forpose(pose);
			} else {
	...
	private void nmod() {
		for (Iterator<MD> i = nmod.iterator(); i.hasNext();) {
			...
			try {
				if (md.real == null) {
					FastMesh.MeshRes mr = md.mod.get().layer(
							FastMesh.MeshRes.class);
					md.real = new Model(mr.m);
					/*
					 * This is really ugly, but I can't really think of anything
					 * less ugly right now.
					 */
					if (md.mod.get().name.equals("gfx/borka/male")
							|| md.mod.get().name.equals("gfx/borka/female"))
						md.real.z = -1;
					this.mod.add(md.real);
				}
				for (Iterator<Indir<Resource>> o = md.tex.iterator(); o
						.hasNext();) {
