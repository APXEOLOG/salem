...
public class OCache implements Iterable<Gob> {
	...
	public synchronized void cres(Gob g, Indir<Resource> res, Message sdt) {
		...
		if ((d == null) || (d.res != res) || (d.sdt.blob.length > 0)
				|| (sdt.blob.length > 0)) {
	...
	public synchronized void cmppose(Gob g, int pseq,
			List<Indir<Resource>> poses, List<Indir<Resource>> tposes,
			boolean interp, float ttime) {
		Composite cmp = (Composite) g.getattr(Drawable.class);
		if (cmp.pseq != pseq) {
			cmp.pseq = pseq;
			if (poses != null)
	...
	public synchronized void follow(Gob g, long oid, Indir<Resource> xfres,
			String xfname) {
		if (oid == 0xffffffffl) {
	...
	public synchronized void overlay(Gob g, int olid, boolean prs,
			Indir<Resource> resid, Message sdt) {
