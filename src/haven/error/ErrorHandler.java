...
public class ErrorHandler extends ThreadGroup {
	...
	public static ErrorHandler find() {
		for (ThreadGroup tg = Thread.currentThread().getThreadGroup(); tg != null; tg = tg
				.getParent()) {
	...
	private class Reporter extends Thread {
		...
		private void doreport(Report r) throws IOException {
			if (!status.goterror(r.t))
				return;
			HttpURLConnection conn = (HttpURLConnection) errordest.openConnection();
			status.connecting();
			
			byte[] params = r.toString().getBytes("UTF-8");
