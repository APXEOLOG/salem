...
public class MeshBuf {
	...
	public Vertex[] copy(FastMesh src) {
		...
		for (int i = 0; i < src.num * 3; i++) {
			...
			if (vmap[idx - min] == null) {
				int o = idx * posb.n;
				Coord3f pos = new Coord3f(posb.data.get(o),
						posb.data.get(o + 1), posb.data.get(o + 2));
				o = idx * nrmb.n;
				Coord3f nrm = new Coord3f(nrmb.data.get(o),
						nrmb.data.get(o + 1), nrmb.data.get(o + 2));
				Coord3f tex = null;
				if (texb != null) {
					o = idx * texb.n;
		...
		for (int i = 0; i < src.num; i++) {
			int o = i * 3;
			new Face(vmap[src.indb.get(o) - min], vmap[src.indb.get(o + 1)
					- min], vmap[src.indb.get(o + 2) - min]);
		}
	...
	private void mkvbuf() {
		...
		if (!hastex)
			this.vbuf = new VertexBuf(new VertexBuf.VertexArray(pos),
					new VertexBuf.NormalArray(nrm));
		else
			this.vbuf = new VertexBuf(new VertexBuf.VertexArray(pos),
					new VertexBuf.NormalArray(nrm), new VertexBuf.TexelArray(
							tex));
	}
