...
public class Utils {
	public static final java.nio.charset.Charset utf8 = java.nio.charset.Charset
			.forName("UTF-8");
	public static final java.nio.charset.Charset ascii = java.nio.charset.Charset
			.forName("US-ASCII");
	public static final java.awt.image.ColorModel rgbm = java.awt.image.ColorModel
			.getRGBdefault();
	...
	static Coord getprefc(String prefname, Coord def) {
		try {
			...
			if (x < 0)
				return (def);
			return (new Coord(Integer.parseInt(val.substring(0, x)),
					Integer.parseInt(val.substring(x + 1))));
		} catch (SecurityException e) {
			return (def);
		}
	}
	...
	static long uint32d(byte[] buf, int off) {
		return (ub(buf[off]) + (ub(buf[off + 1]) * 256)
				+ (ub(buf[off + 2]) * 65536) + (ub(buf[off + 3]) * 16777216l));
	}
	...
	public static BufferedImage monochromize(BufferedImage img, Color col) {
		...
		for (int y = 0; y < sz.y; y++) {
			for (int x = 0; x < sz.x; x++) {
				...
				int r = rgbm.getRed(px), g = rgbm.getGreen(px), b = rgbm
						.getBlue(px), a = rgbm.getAlpha(px);
	...
	public static ShortBuffer mksbuf(int n) {
		return (ByteBuffer.allocateDirect(n * 2).order(ByteOrder.nativeOrder())
				.asShortBuffer());
	}
	...
	public static <T> T[] splice(T[] src, int off, int len) {
		T[] dst = (T[]) java.lang.reflect.Array.newInstance(src.getClass()
