...
public class RichText extends Text {
	...
	public static class Image extends Part {
		...
		public Image(Resource res, int id) {
			...
			if (this.img == null)
				throw (new RuntimeException("Found no image with id " + id
						+ " in " + res.toString()));
		}
	...
	public static class TextPart extends Part {
		...
		public TextPart(String str, Map<? extends Attribute, ?> attrs) {
			this((str.length() == 0) ? (new AttributedString(str))
					: (new AttributedString(str, attrs)), 0, str.length());
		}
		...
		public int height() {
			if (start == end)
				return (0);
			return ((int) (tl().getAscent() + tl().getDescent() + tl()
					.getLeading()));
		}
	 ...
	 */
	private static Map<? extends Attribute, ?> fixattrs(
	...
	public static class Parser {
		...
		private static String name(PeekReader in) throws IOException {
			...
			if (buf.length() == 0)
				throw (new FormatException("Expected name, got `"
						+ (char) in.peek() + "'"));
			return (buf.toString());
		}
		...
		private static Part tag(PState s, Map<? extends Attribute, ?> attrs)
				throws IOException {
			s.in.peek(true);
				...
				while (true) {
					...
					if (c < 0) {
						throw (new FormatException(
								"Unexpected end-of-input when reading tag arguments"));
					} else if (c == ']') {
						s.in.read();
						break;
					} else {
			...
			if (tn == "img") {
				...
				Map<Attribute, Object> na = new HashMap<Attribute, Object>(
						attrs);
				if (tn == "font") {
		...
		private static Part text(PState s, Map<? extends Attribute, ?> attrs)
				throws IOException {
		...
		private static Part parse(PState s, Map<? extends Attribute, ?> attrs)
				throws IOException {
			Part res = text(s, attrs);
			if (s.in.peek() >= 0)
				throw (new FormatException("Junk left after the end of input: "
						+ (char) s.in.peek()));
			return (res);
		}

		public Part parse(Reader in, Map<? extends Attribute, ?> extra)
				throws IOException {
	...
	public static class Foundry {
		...
		private static void aline/* Hurrhurr, pun intended */(List<Part> line,
				int y) {
			int mb = 0;
			for (Part p : line) {
				int cb = p.baseline();
	...
	public static void main(String[] args) throws Exception {
		...
		if (cmd == "render") {
			...
			if (p == null)
				throw (new Exception("No pagina in " + res + ", loaded from "
						+ res.source));
