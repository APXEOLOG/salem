...
public class Window extends Widget implements DTarget {
	...
	public static final IBox swbox = new IBox("gfx/hud", "stl", "str", "sbl",
			"sbr", "sextv", "sextv", "sexth", "sexth");
	private static final IBox wbox = new IBox("gfx/hud", "tl", "tr", "bl",
			"br", "extvl", "extvr", "extht", "exthb");
	boolean dt = false;
	Text cap;
	boolean dm = false;
...
//	}
	...
	private void placecbtn() {
		cbtn.c = xlate(
				new Coord(tlo.x + wsz.x - Utils.imgsz(cbtni[0]).x, tlo.y),
				false);
	}
	...
	public void draw(GOut og) {
		...
		if (wsz.x > wbox.ctl.sz().x + wbox.ctr.sz().x + 0 + topblob.sz().x) {
			...
			g.image(botblob, new Coord((wsz.x - botblob.sz().x) / 2, wsz.y
					- botblob.sz().y));
		}
		/*
