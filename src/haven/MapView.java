...
public class MapView extends PView implements DTarget {
	...
	private class FollowCam extends Camera {
		...
		public void drag(Coord c) {
			tangl = anglorig + ((c.x - dragorig.x) / 100.0f);
			tangl = tangl % ((float) Math.PI * 2.0f);
		}

		private float dist(float elev) {
			return ((float) (((cd - (h / Math.tan(elev))) * Math.sin(elev - da) / Math
					.sin(da)) - (h / Math.sin(elev))));
		}
		...
		public void tick(double dt) {
			...
			if (dist > 250) {
				...
				Coord3f cambase = new Coord3f(curc.x
						...
						+ ((float) Math.sin(tangl) * pd), 0.0f);
				float a = cc.xyangle(curc);
				float nx = cc.x + ((float) Math.cos(a) * fr), ny = cc.y
						+ ((float) Math.sin(a) * fr);
				Coord3f tgtc = new Coord3f(nx, ny, cc.z);
				curc = curc.add(tgtc.sub(curc).mul(
						(float) (1.0 - Math.pow(500, -dt))));
		...
		public Matrix4f compute() {
			return (PointedCam.compute(curc.add(0.0f, 0.0f, h), dist(elev),
					elev, angl));
		}
	...
	private class FreeCam extends Camera {
		...
		public Matrix4f compute() {
			Coord3f cc = getcc();
			cc.y = -cc.y;
			return (PointedCam.compute(cc.add(0.0f, 0.0f, 15f), dist, elev,
					angl));
		}
		...
		@Override
		public void drag(Coord c) {
			elev = elevorig - ((c.y - dragorig.y) / 100.0f);
			if (elev < 0.0f)
				elev = 0.0f;
			if (elev > (Math.PI / 2.0))
				elev = (float) Math.PI / 2.0f;
			angl = anglorig + ((c.x - dragorig.x) / 100.0f);
	...
	public void setup(RenderList rl) {
		...
		synchronized (glob) {
			if (glob.lightamb != null) {
				DirLight light = new DirLight(glob.lightamb, glob.lightdif,
						glob.lightspc, Coord3f.o.sadd((float) glob.lightelev,
								(float) glob.lightang, 1f));
				rl.add(light, null);
				if (rl.cfg.deflight == Light.pslights) {
					if (smap == null)
						smap = new Light.PSLights.ShadowMap(new Coord(2048,
								2048), 750, 5000);
					smap.light = light;
					Coord3f dir = new Coord3f(-light.dir[0], -light.dir[1],
							-light.dir[2]);
					Coord3f cc = getcc();
					cc.y = -cc.y;
	...
	private abstract static class Clicklist<T> extends RenderList {
		...
		protected Color newcol(T t) {
			int cr = ((i & 0x00000f) << 4) | ((i & 0x00f000) >> 12), cg = ((i & 0x0000f0) << 0)
					| ((i & 0x0f0000) >> 16), cb = ((i & 0x000f00) >> 4)
					| ((i & 0xf00000) >> 20);
	...
	private Coord checkmapclick(GOut g, Coord c) {
		...
		Coord pixel;
		{
			...
			if (hitcol.getBlue() != 0)
				return (null);
			pixel = new Coord((hitcol.getRed() * tilesz.x) / 255,
					(hitcol.getGreen() * tilesz.y) / 255);
		}
	...
	private void drawarrow(GOut g, double a) {
		...
		if ((a > ca) && (a < -ca)) {
			...
			ac = new Coord(hsz.x + (int) (Math.tan(a + Math.PI / 2) * hsz.y),
					sz.y);
		} else {
			...
			delayed.add(new Hittest(cc) {
				...
				public void hit(Coord pc, Coord mc, Gob gob, Rendered tgt) {
					...
					if (gob == null)
						...
						wdgmsg("itemact", pc, mc, ui.modflags(), (int) gob.id,
								gob.rc, getid(tgt));
				}
			});
		}
