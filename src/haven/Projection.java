...
public class Projection extends Transform {
	...
	public Coord3f toscreen(Coord3f ec, Coord sz) {
		Coord3f n = tonorm(ec);
		return (new Coord3f(((n.x + 1) / 2) * sz.x, ((-n.y + 1) / 2) * sz.y,
				n.z));
	}

	public static Matrix4f makefrustum(Matrix4f d, float left, float right,
	...
	public static Projection frustum(float left, float right, float bottom,
			float top, float near, float far) {
		return (new Projection(makefrustum(new Matrix4f(), left, right, bottom,
				top, near, far)));
	}

	public static Matrix4f makeortho(Matrix4f d, float left, float right,
			float bottom, float top, float near, float far) {
		d.m[0] = 2 / (right - left);
		d.m[5] = 2 / (top - bottom);
		d.m[10] = -2 / (far - near);
	...
	public static Projection ortho(float left, float right, float bottom,
			float top, float near, float far) {
		return (new Projection(makeortho(new Matrix4f(), left, right, bottom,
				top, near, far)));
	}
