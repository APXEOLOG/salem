...
public class RenderList {
	...
	public void add(Rendered r, GLState t) {
		...
		if (curp == null)
			throw (new RuntimeException(
					"Tried to set up relative slot with no parent"));
		curp.cs.copy(s.os);
		if (t != null)
			t.prep(s.os);
		s.os.copy(s.cs);
	...
	private static final Comparator<Slot> cmp = new Comparator<Slot>() {
		public int compare(Slot a, Slot b) {
			...
			if (a.o != b.o)
				throw (new RuntimeException(
						"Found two different orderings with the same main-Z: "
								+ a.o + " and " + b.o));
		...
		for (int i = 0; i < cur; i++) {
			...
			for (GLState st : sl) {
				if (st instanceof GLState.GlobalState) {
					GLState.Global gst = ((GLState.GlobalState) st).global(
							this, ctx);
					gstates.put(gst, gst);
				}
			}
	...
	public void rewind() {
		if (curp != null)
			throw (new RuntimeException(
					"Tried to rewind RenderList while adding to it."));
		cur = 0;
	}
