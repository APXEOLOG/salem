...
public class MapMesh implements Rendered {
	...
	private static final Material.Colors gcol = new Material.Colors(new Color(
			128, 128, 128), new Color(255, 255, 255), new Color(0, 0, 0),
			new Color(0, 0, 0));
	...
	public GLState stfor(Tex tex, boolean clip) {
		...
		if (tex instanceof TexGL)
			gt = (TexGL) tex;
		else if ((tex instanceof TexSI)
				&& (((TexSI) tex).parent instanceof TexGL))
			gt = (TexGL) ((TexSI) tex).parent;
		else
			throw (new RuntimeException(
					"Cannot use texture for map rendering: " + tex));
		...
		public Surface() {
			...
			for (c.y = -1; c.y <= sz.y + 1; c.y++) {
				for (c.x = -1; c.x <= sz.x + 1; c.x++) {
					surf[i++] = new SPoint(new Coord3f(c.x * tilesz.x, c.y
							* -tilesz.y, map.getz(ul.add(c))));
				}
			}
		}
		...
		public void calcnrm() {
			...
			for (c.y = 0; c.y <= sz.y; c.y++) {
				for (c.x = 0; c.x <= sz.x; c.x++) {
					...
					Coord3f nrm = (n.cmul(w)).add(e.cmul(n)).add(s.cmul(e))
							.add(w.cmul(s)).norm();
					p.nrm = nrm;
					i++;
				}
	...
	private static SPoint[] fortile(Surface surf, Coord sc) {
		return (new SPoint[] { surf.spoint(sc), surf.spoint(sc.add(0, 1)),
				surf.spoint(sc.add(1, 1)), surf.spoint(sc.add(1, 0)), });
	}
	...
	public <T extends Surface> T surf(Class<T> cl) {
		...
		if (ret == null) {
			try {
				java.lang.reflect.Constructor<T> c = cl
						.getConstructor(MapMesh.class);
	...
	public static class GroundMod implements Rendered {
		...
		public GroundMod(MCache map, Class<? extends Surface> surf, Tex tex,
				Coord cc, Coord ul, Coord br) {
			...
			for (t.y = 0; t.y < brt.y - ult.y; t.y++) {
				for (t.x = 0; t.x < brt.x - ult.x; t.x++) {
					splitquad(buf, vm[t.x][t.y], vm[t.x][t.y + 1],
							vm[t.x + 1][t.y + 1], vm[t.x + 1][t.y]);
				}
			}
			mesh = buf.mkmesh();
		}
	...
	public void drawflat(GOut g, int mode) {
		...
		for (c.y = 0; c.y < sz.y; c.y++) {
			for (c.x = 0; c.x < sz.x; c.x++) {
				...
				gl.glVertex3f(c.x * tilesz.x, (c.y + 1) * -tilesz.y,
						fz(c.x, c.y + 1));
				if (mode == 2)
					gl.glColor3f(1.0f, 1.0f, 0.0f);
				gl.glVertex3f((c.x + 1) * tilesz.x, (c.y + 1) * -tilesz.y,
						fz(c.x + 1, c.y + 1));
				if (mode == 2)
					gl.glColor3f(0.0f, 0.0f, 0.0f);
				gl.glVertex3f(c.x * tilesz.x, c.y * -tilesz.y, fz(c.x, c.y));
				...
				gl.glVertex3f((c.x + 1) * tilesz.x, c.y * -tilesz.y,
						fz(c.x + 1, c.y));
			}
		}
		gl.glEnd();
