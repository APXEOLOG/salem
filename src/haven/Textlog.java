...
public class Textlog extends Widget {
	...
	static RichText.Foundry fnd = new RichText.Foundry(TextAttribute.FAMILY,
			"SansSerif", TextAttribute.SIZE, 9, TextAttribute.FOREGROUND,
			Color.BLACK);
	List<Text> lines;
	int maxy, cury;
	int margin = 3;
	...
	public void append(String line, Color col) {
		Text rl;
		if (col == null)
			rl = fnd.render(RichText.Parser.quote(line), sz.x - (margin * 2)
					- sflarp.sz().x);
		else
			rl = fnd.render(RichText.Parser.quote(line), sz.x - (margin * 2)
					- sflarp.sz().x, TextAttribute.FOREGROUND, col);
		synchronized (lines) {
			lines.add(rl);
		}
		if (cury == maxy)
	...
	public void mousemove(Coord c) {
		if (sdrag) {
			double a = (double) (c.y - (sflarp.sz().y / 2))
					/ (double) (sz.y - sflarp.sz().y);
			if (a < 0)
				a = 0;
			if (a > 1)
				a = 1;
