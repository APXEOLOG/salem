...
public abstract class TexGL extends Tex {
	...
	protected int magfilter = GL.GL_NEAREST, minfilter = GL.GL_NEAREST,
			wrapmode = GL.GL_REPEAT;
	...
	public static class TexDraw extends GLState {
		public static final Slot<TexDraw> slot = new Slot<TexDraw>(
				Slot.Type.DRAW, TexDraw.class, HavenPanel.global);
			...
			if (clip != null) {
				if (clip.tex != this.tex)
					throw (new RuntimeException(
							"TexGL does not support different clip and draw textures."));
	...
	public static class TexClip extends GLState {
		public static final Slot<TexClip> slot = new Slot<TexClip>(
				Slot.Type.GEOM, TexClip.class, HavenPanel.global, TexDraw.slot);
		public final TexGL tex;
		...
		public void apply(GOut g) {
			...
			if (draw == null) {
				...
				gl.glTexEnvi(GL.GL_TEXTURE_ENV, GL.GL_COMBINE_ALPHA,
						GL.GL_MODULATE);
				gl.glTexEnvi(GL.GL_TEXTURE_ENV, GL.GL_SRC0_ALPHA,
						GL.GL_PREVIOUS);
		...
		public void applyfrom(GOut g, GLState from) {
			...
			if ((old == null) && (draw == null)) {
				...
				throw (new RuntimeException(
						"TexClip is somehow being transition even though TexDraw is being replaced"));
			}
		}
